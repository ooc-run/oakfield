#include "oakfield/operators/stimulus/hybrid_fbm.h"
...
#define STIM_HYBRID_FBM_MAX_OCTAVES 16U

typedef struct SimStimulusHybridFbmState {
    SimStimulusHybridFbmConfig        config;
    double*                           phases;
    unsigned int                      allocated_octaves;
    SimStimulusStaticCache            cache;
    SimStimulusOctaveNoiseVdspBuffers vdsp;
    char                              symbolic[160];
} SimStimulusHybridFbmState;

static void hybrid_fbm_normalize(SimStimulusHybridFbmConfig* config) {
    if (config == NULL) {
        return;
    }
    ...
    if (config->octaves > STIM_HYBRID_FBM_MAX_OCTAVES) {
        config->octaves = STIM_HYBRID_FBM_MAX_OCTAVES;
    }
    if (config->seed == 0ULL) {
        config->seed = 1ULL;
    }
...
static void hybrid_fbm_refresh_symbolic(SimStimulusHybridFbmState* state) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    if (state == NULL) {
        return;
    }
    const SimStimulusHybridFbmConfig* cfg = &state->config;
    (void) snprintf(state->symbolic,
                    sizeof(state->symbolic),
                    "hybrid_fbm A=%.3g H=%.3g O=%u",
                    cfg->amplitude,
                    cfg->hurst,
                    cfg->octaves);
#else
...
static const char* hybrid_fbm_symbolic(const void* state_ptr) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    const SimStimulusHybridFbmState* state = (const SimStimulusHybridFbmState*) state_ptr;
...
static void hybrid_fbm_eval_base(const SimStimulusHybridFbmState* state,
                                 double                           u,
                                 double                           hurst,
                                 double                           lacunarity,
                                 ...
                                 double*                          out_re,
                                 double*                          out_im) {
    sim_stimulus_octave_noise_eval_base(SIM_STIMULUS_OCTAVE_NOISE_HYBRID_FBM,
...
static void hybrid_fbm_destroy(void* state_ptr) {
    SimStimulusHybridFbmState* state = (SimStimulusHybridFbmState*) state_ptr;
    if (state == NULL) {
        return;
    }
...
static SimResult hybrid_fbm_ensure_octaves(SimStimulusHybridFbmState* state) {
    if (state == NULL) {
...
static bool hybrid_fbm_can_use_static_cache(const SimStimulusHybridFbmState* state) {
    return state != NULL && sim_stimulus_coord_is_time_invariant(&state->config.coord);
}

static SimResult hybrid_fbm_fill_static_cache(void*                               userdata,
                                              const SimStimulusStaticCacheLayout* layout,
                                              bool                                need_imag,
                                              double*                             out_real,
                                              double*                             out_imag) {
    SimStimulusHybridFbmState* state = (SimStimulusHybridFbmState*) userdata;
    ...
    for (size_t idx = 0U; idx < layout->count; ++idx) {
        ...
        if (separable) {
            ...
            double fy_re = 0.0;
            double fy_im = 0.0;
            hybrid_fbm_eval_base(state, sample_x, hurst, lacunarity, octaves, &fx_re, &fx_im);
            hybrid_fbm_eval_base(state, sample_y, hurst, lacunarity, octaves, &fy_re, &fy_im);
            ...
            hybrid_fbm_eval_base(state, u, hurst, lacunarity, octaves, &base_re, &base_im);
        }
...
static SimResult hybrid_fbm_ensure_static_cache(SimStimulusHybridFbmState*          state,
                                                const SimStimulusStaticCacheLayout* layout,
                                                bool                                need_imag) {
    ...
    SimResult prep = hybrid_fbm_ensure_octaves(state);
    if (prep != SIM_RESULT_OK) {
        return prep;
    }

    return sim_stimulus_static_cache_ensure(
        &state->cache, layout, need_imag, hybrid_fbm_fill_static_cache, state);
}
...
static SimResult hybrid_fbm_kernel_value(SimStimulusHybridFbmState* state,
    ...
    SimStimulusHybridFbmConfig* cfg = &state->config;
    ...
    if (hybrid_fbm_can_use_static_cache(state)) {
        SimResult prep = hybrid_fbm_ensure_static_cache(state, &layout, use_imag);
        if (prep != SIM_RESULT_OK) {
            return prep;
        }
        double value =
...
static SimResult hybrid_fbm_ir_eval(void*           userdata,
                                    const KernelIR* kernel,
                                    size_t          element_index,
                                    size_t          component,
                                    double*         out_value) {
    ...
    SimStimulusHybridFbmState* state = (SimStimulusHybridFbmState*) userdata;
    return hybrid_fbm_kernel_value(state, kernel, element_index, false, out_value);
}

static SimResult hybrid_fbm_ir_eval_imag(void*           userdata,
                                         const KernelIR* kernel,
                                         size_t          element_index,
                                         size_t          component,
                                         double*         out_value) {
    ...
    SimStimulusHybridFbmState* state = (SimStimulusHybridFbmState*) userdata;
    return hybrid_fbm_kernel_value(state, kernel, element_index, true, out_value);
}

static SimResult hybrid_fbm_step(void*               state_ptr,
    ...
    if (hybrid_fbm_can_use_static_cache(state)) {
        SimResult prep = hybrid_fbm_ensure_static_cache(state, &layout, is_complex);
        if (prep != SIM_RESULT_OK) {
            return prep;
        }
    ...
    if (sim_stimulus_octave_noise_try_vdsp_rows(SIM_STIMULUS_OCTAVE_NOISE_HYBRID_FBM,
                                                &state->vdsp,
                                                &cfg->coord,
                                                state->phases,
                                                cfg->amplitude,
...
SimResult sim_add_stimulus_hybrid_fbm_operator(struct SimContext*                context,
    ...
    SimStimulusHybridFbmConfig local = { 0 };
    if (config != NULL) {
        local = *config;
    }
    ...
    if (local.seed == 0ULL) {
        local.seed = sim_seed_derive(sim_context_seed(context),
                                     sim_seed_tag("stimulus_hybrid_fbm"),
                                     sim_context_operator_count(context));
    }

    hybrid_fbm_normalize(&local);
    local.scale_by_dt =
    ...
    SimStimulusHybridFbmState* state =
        (SimStimulusHybridFbmState*) calloc(1U, sizeof(SimStimulusHybridFbmState));
    if (state == NULL) {
    ...
    hybrid_fbm_refresh_symbolic(state);

    char name[SIM_OPERATOR_NAME_MAX + 1U];
    sim_operator_make_unique_name(name, sizeof(name), "stimulus_hybrid_fbm");
    ...
    info.abstract_id       = "stimulus_hybrid_fbm";
    sim_operator_info_set_schema_identity(&info, "stimulus_hybrid_fbm");
    ...
    if (sim_operator_should_register_kernel_for_schema(
            context, &op_config, 0ULL, SIM_DET_NONE, "stimulus_hybrid_fbm")) {
        ...
        if (field != NULL) {
            ...
            if (builder != NULL) {
                ...
                SimIRNodeId sample_node = sim_ir_builder_stateful(
                    builder, hybrid_fbm_ir_eval, state, "stimulus_hybrid_fbm");
                if (is_complex && sample_node != SIM_IR_INVALID_NODE) {
                    SimIRNodeId sample_im = sim_ir_builder_stateful(
                        builder, hybrid_fbm_ir_eval_imag, state, "stimulus_hybrid_fbm_im");
                ...
                if (field_node != SIM_IR_INVALID_NODE && sample_node != SIM_IR_INVALID_NODE &&
                    ...
                    kdesc.name                  = name;
                    kdesc.evaluate              = NULL;
                    kdesc.destroy               = hybrid_fbm_destroy;
    ...
    SimSplitDescriptor desc = { .name          = name,
                                ...
                                .substep_count = 1U,
                                .state         = state,
                                .symbolic      = hybrid_fbm_symbolic,
                                .destroy       = hybrid_fbm_destroy,
                                .info          = info,
                                .config        = op_config,
                                .scratch       = { 0U, 0U } };
    ...
    if (result != SIM_RESULT_OK) {
        hybrid_fbm_destroy(state);
    }
    return result;
}

SimResult sim_stimulus_hybrid_fbm_config(struct SimContext*          context,
    ...
    SimStimulusHybridFbmState* state = NULL;
    if (op->kernel != NULL) {
        state = (SimStimulusHybridFbmState*) sim_operator_payload(op);
    } else {
        state = (SimStimulusHybridFbmState*) sim_split_state(op);
    }
    if (state == NULL) {
...
SimResult sim_stimulus_hybrid_fbm_update(struct SimContext*                context,
                                         size_t                            operator_index,
                                         const SimStimulusHybridFbmConfig* config) {
    if (context == NULL) {
        return SIM_RESULT_INVALID_ARGUMENT;
    ...
    SimStimulusHybridFbmConfig local = state->config;
    if (config != NULL) {
        local = *config;
    }
    ...
    if (local.seed == 0ULL) {
        local.seed = sim_seed_derive(
            sim_context_seed(context), sim_seed_tag("stimulus_hybrid_fbm"), operator_index);
    }

    hybrid_fbm_normalize(&local);
    local.scale_by_dt = sim_operator_resolve_scale_by_dt(
        context, sim_operator_schema_key_or(op, "stimulus_hybrid_fbm"), true, local.scale_by_dt);
    state->config            = local;
    state->allocated_octaves = 0U;
