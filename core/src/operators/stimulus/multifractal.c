#include "oakfield/operators/stimulus/multifractal.h"
...
#define STIM_MULTIFRACTAL_MAX_OCTAVES 16U

typedef struct SimStimulusMultifractalState {
    SimStimulusMultifractalConfig     config;
    double*                           phases;
    unsigned int                      allocated_octaves;
    SimStimulusStaticCache            cache;
    SimStimulusOctaveNoiseVdspBuffers vdsp;
    char                              symbolic[160];
} SimStimulusMultifractalState;

static void multifractal_normalize(SimStimulusMultifractalConfig* config) {
    if (config == NULL) {
        return;
    }
    ...
    if (config->octaves > STIM_MULTIFRACTAL_MAX_OCTAVES) {
        config->octaves = STIM_MULTIFRACTAL_MAX_OCTAVES;
    }
    if (config->seed == 0ULL) {
        config->seed = 1ULL;
    }
...
static void multifractal_refresh_symbolic(SimStimulusMultifractalState* state) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    if (state == NULL) {
        return;
    }
    const SimStimulusMultifractalConfig* cfg = &state->config;
    (void) snprintf(state->symbolic,
                    sizeof(state->symbolic),
                    "multifractal A=%.3g H=%.3g O=%u",
                    cfg->amplitude,
                    cfg->hurst,
                    cfg->octaves);
#else
...
static const char* multifractal_symbolic(const void* state_ptr) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    const SimStimulusMultifractalState* state = (const SimStimulusMultifractalState*) state_ptr;
...
static void multifractal_eval_base(const SimStimulusMultifractalState* state,
                                   double                              u,
                                   double                              hurst,
                                   double                              lacunarity,
                                   ...
                                   double*                             out_re,
                                   double*                             out_im) {
    sim_stimulus_octave_noise_eval_base(SIM_STIMULUS_OCTAVE_NOISE_MULTIFRACTAL,
...
static void multifractal_destroy(void* state_ptr) {
    SimStimulusMultifractalState* state = (SimStimulusMultifractalState*) state_ptr;
    if (state == NULL) {
        return;
    }
...
static SimResult multifractal_ensure_octaves(SimStimulusMultifractalState* state) {
    if (state == NULL) {
...
static bool multifractal_can_use_static_cache(const SimStimulusMultifractalState* state) {
    return state != NULL && sim_stimulus_coord_is_time_invariant(&state->config.coord);
}

static SimResult multifractal_fill_static_cache(void*                               userdata,
                                                const SimStimulusStaticCacheLayout* layout,
                                                bool                                need_imag,
                                                double*                             out_real,
                                                double*                             out_imag) {
    SimStimulusMultifractalState* state = (SimStimulusMultifractalState*) userdata;
    ...
    for (size_t idx = 0U; idx < layout->count; ++idx) {
        ...
        if (separable) {
            ...
            double fy_re = 0.0;
            double fy_im = 0.0;
            multifractal_eval_base(state, sample_x, hurst, lacunarity, octaves, &fx_re, &fx_im);
            multifractal_eval_base(state, sample_y, hurst, lacunarity, octaves, &fy_re, &fy_im);
            ...
            multifractal_eval_base(state, u, hurst, lacunarity, octaves, &base_re, &base_im);
        }
...
static SimResult multifractal_ensure_static_cache(SimStimulusMultifractalState*       state,
                                                  const SimStimulusStaticCacheLayout* layout,
                                                  bool                                need_imag) {
    ...
    SimResult prep = multifractal_ensure_octaves(state);
    if (prep != SIM_RESULT_OK) {
        return prep;
    }

    return sim_stimulus_static_cache_ensure(
        &state->cache, layout, need_imag, multifractal_fill_static_cache, state);
}
...
static SimResult multifractal_kernel_value(SimStimulusMultifractalState* state,
    ...
    SimStimulusMultifractalConfig* cfg = &state->config;
    ...
    if (multifractal_can_use_static_cache(state)) {
        SimResult prep = multifractal_ensure_static_cache(state, &layout, use_imag);
        if (prep != SIM_RESULT_OK) {
            return prep;
        }
        double value =
...
static SimResult multifractal_ir_eval(void*           userdata,
                                      const KernelIR* kernel,
                                      size_t          element_index,
                                      size_t          component,
                                      double*         out_value) {
    ...
    SimStimulusMultifractalState* state = (SimStimulusMultifractalState*) userdata;
    return multifractal_kernel_value(state, kernel, element_index, false, out_value);
}

static SimResult multifractal_ir_eval_imag(void*           userdata,
                                           const KernelIR* kernel,
                                           size_t          element_index,
                                           size_t          component,
                                           double*         out_value) {
    ...
    SimStimulusMultifractalState* state = (SimStimulusMultifractalState*) userdata;
    return multifractal_kernel_value(state, kernel, element_index, true, out_value);
}

static SimResult multifractal_step(void*               state_ptr,
    ...
    if (multifractal_can_use_static_cache(state)) {
        SimResult prep = multifractal_ensure_static_cache(state, &layout, is_complex);
        if (prep != SIM_RESULT_OK) {
            return prep;
        }
    ...
    if (sim_stimulus_octave_noise_try_vdsp_rows(SIM_STIMULUS_OCTAVE_NOISE_MULTIFRACTAL,
                                                &state->vdsp,
                                                &cfg->coord,
                                                state->phases,
                                                cfg->amplitude,
...
SimResult sim_add_stimulus_multifractal_operator(struct SimContext*                   context,
    ...
    SimStimulusMultifractalConfig local = { 0 };
    if (config != NULL) {
        local = *config;
    }
    ...
    if (local.seed == 0ULL) {
        local.seed = sim_seed_derive(sim_context_seed(context),
                                     sim_seed_tag("stimulus_multifractal"),
                                     sim_context_operator_count(context));
    }

    multifractal_normalize(&local);
    local.scale_by_dt =
        sim_operator_resolve_scale_by_dt(context,
                                         "stimulus_multifractal",
                                         (config != NULL),
                                         (config != NULL) ? config->scale_by_dt : true);

    SimStimulusMultifractalState* state =
        (SimStimulusMultifractalState*) calloc(1U, sizeof(SimStimulusMultifractalState));
    if (state == NULL) {
    ...
    multifractal_refresh_symbolic(state);

    char name[SIM_OPERATOR_NAME_MAX + 1U];
    sim_operator_make_unique_name(name, sizeof(name), "stimulus_multifractal");
    ...
    info.abstract_id       = "stimulus_multifractal";
    sim_operator_info_set_schema_identity(&info, "stimulus_multifractal");
    ...
    if (sim_operator_should_register_kernel_for_schema(
            context, &op_config, 0ULL, SIM_DET_NONE, "stimulus_multifractal")) {
        ...
        if (field != NULL) {
            ...
            if (builder != NULL) {
                ...
                SimIRNodeId sample_node = sim_ir_builder_stateful(
                    builder, multifractal_ir_eval, state, "stimulus_multifractal");
                if (is_complex && sample_node != SIM_IR_INVALID_NODE) {
                    SimIRNodeId sample_im = sim_ir_builder_stateful(
                        builder, multifractal_ir_eval_imag, state, "stimulus_multifractal_im");
                ...
                if (field_node != SIM_IR_INVALID_NODE && sample_node != SIM_IR_INVALID_NODE &&
                    ...
                    kdesc.name                  = name;
                    kdesc.evaluate              = NULL;
                    kdesc.destroy               = multifractal_destroy;
    ...
    SimSplitDescriptor desc = { .name          = name,
                                ...
                                .substep_count = 1U,
                                .state         = state,
                                .symbolic      = multifractal_symbolic,
                                .destroy       = multifractal_destroy,
                                .info          = info,
                                .config        = op_config,
                                .scratch       = { 0U, 0U } };
    ...
    if (result != SIM_RESULT_OK) {
        multifractal_destroy(state);
    }
    return result;
}

SimResult sim_stimulus_multifractal_config(struct SimContext*             context,
    ...
    SimStimulusMultifractalState* state = NULL;
    if (op->kernel != NULL) {
        state = (SimStimulusMultifractalState*) sim_operator_payload(op);
    } else {
        state = (SimStimulusMultifractalState*) sim_split_state(op);
    }
    if (state == NULL) {
...
SimResult sim_stimulus_multifractal_update(struct SimContext*                   context,
                                           size_t                               operator_index,
                                           const SimStimulusMultifractalConfig* config) {
    if (context == NULL) {
        return SIM_RESULT_INVALID_ARGUMENT;
    ...
    SimStimulusMultifractalConfig local = state->config;
    if (config != NULL) {
        local = *config;
    }
    ...
    if (local.seed == 0ULL) {
        local.seed = sim_seed_derive(
            sim_context_seed(context), sim_seed_tag("stimulus_multifractal"), operator_index);
    }

    multifractal_normalize(&local);
    local.scale_by_dt = sim_operator_resolve_scale_by_dt(
        context, sim_operator_schema_key_or(op, "stimulus_multifractal"), true, local.scale_by_dt);
    state->config            = local;
    state->allocated_octaves = 0U;
