...
#define STIM_OPTICAL_VORTEX_EPS 1.0e-12
#define STIM_OPTICAL_VORTEX_MAX_CHARGE 16
#define STIM_OPTICAL_VORTEX_VDSP_MIN_LEN 64U

typedef struct SimStimulusOpticalVortexState {
    SimStimulusOpticalVortexConfig config;
    char                           symbolic[256];
...
#endif
} SimStimulusOpticalVortexState;

static void optical_vortex_normalize(SimStimulusOpticalVortexConfig* config) {
    if (config == NULL) {
        return;
    }
    ...
    if (!isfinite(config->waist_x) || fabs(config->waist_x) <= STIM_OPTICAL_VORTEX_EPS) {
        config->waist_x = 0.75;
    }
    if (!isfinite(config->waist_y) || fabs(config->waist_y) <= STIM_OPTICAL_VORTEX_EPS) {
        config->waist_y = config->waist_x;
    }
    config->waist_x = fabs(config->waist_x);
    config->waist_y = fabs(config->waist_y);

    if (config->charge > STIM_OPTICAL_VORTEX_MAX_CHARGE) {
        config->charge = STIM_OPTICAL_VORTEX_MAX_CHARGE;
    } else if (config->charge < -STIM_OPTICAL_VORTEX_MAX_CHARGE) {
        config->charge = -STIM_OPTICAL_VORTEX_MAX_CHARGE;
    } else if (config->charge == 0) {
        config->charge = 1;
    }
...
static void optical_vortex_refresh_symbolic(SimStimulusOpticalVortexState* state) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    if (state == NULL) {
        return;
    }

    const SimStimulusOpticalVortexConfig* cfg = &state->config;
    (void) snprintf(state->symbolic,
                    sizeof(state->symbolic),
                    "optical_vortex A=%.3g l=%d waist=(%.3g,%.3g) center=(%.3g,%.3g)",
                    cfg->amplitude,
                    cfg->charge,
                    cfg->waist_x,
                    cfg->waist_y,
...
static const char* optical_vortex_symbolic(const void* state_ptr) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    const SimStimulusOpticalVortexState* state = (const SimStimulusOpticalVortexState*) state_ptr;
...
static void optical_vortex_map_coord(const SimStimulusOpticalVortexConfig* cfg,
                                     double                                x,
                                     double                                y,
                                     double                                t,
                                     double*                               out_u,
...
static void optical_vortex_eval_uv(const SimStimulusOpticalVortexConfig* cfg,
                                   double                                u,
                                   double                                v,
                                   double                                t,
                                   double*                               out_re,
    ...
    int    abs_charge = (cfg->charge < 0) ? -cfg->charge : cfg->charge;
    if (abs_charge > 0) {
        envelope *= pow(rho, (double) abs_charge);
    }

    double phase = (double) cfg->charge * atan2(yv, xu) - cfg->omega * t + cfg->phase;
    *out_re      = envelope * cos(phase);
    *out_im      = envelope * sin(phase);
}

#if defined(SIM_HAVE_VDSP)
static bool optical_vortex_vdsp_ensure_buffers(SimStimulusOpticalVortexState* state, size_t width) {
...
static bool optical_vortex_linear_map(const SimStimulusOpticalVortexConfig* cfg,
                                      double*                               out_u_x,
                                      double*                               out_u_y,
                                      double*                               out_v_x,
                                      double*                               out_v_y) {
...
static void optical_vortex_rho_factor(double* rho2,
                                      double* out,
                                      double* scratch,
                                      size_t  width,
                                      int     abs_charge) {
    if (rho2 == NULL || out == NULL || scratch == NULL || width == 0U) {
        return;
    }

    const vDSP_Length len = (vDSP_Length) width;
    double            one = 1.0;
    vDSP_vfillD(&one, out, 1, len);

    int half_power = abs_charge / 2;
    for (int power = 0; power < half_power; ++power) {
        vDSP_vmulD(out, 1, rho2, 1, out, 1, len);
    }

    if ((abs_charge & 1) != 0) {
        const int vforce_len = (int) width;
        vvsqrt(scratch, rho2, &vforce_len);
        vDSP_vmulD(out, 1, scratch, 1, out, 1, len);
    }
}

static bool optical_vortex_try_vdsp_linear_rows(SimStimulusOpticalVortexState* state,
    ...
    if (state == NULL || field == NULL ||
        !optical_vortex_linear_map(&state->config, &u_x, &u_y, &v_x, &v_y)) {
        return false;
    }
    ...
    if (width == 0U || height == 0U || width < STIM_OPTICAL_VORTEX_VDSP_MIN_LEN || width > count) {
        return false;
    }
    ...
    if (!optical_vortex_vdsp_ensure_buffers(state, width)) {
        return false;
    }
    ...
    double                                inv_waist_x     = 1.0 / cfg->waist_x;
    double                                inv_waist_y     = 1.0 / cfg->waist_y;
    double                                phase_scale     = (double) cfg->charge;
    ...
    if (!isfinite(x0) || !isfinite(y0) || !isfinite(dx) || !isfinite(dy) || !isfinite(theta) ||
        ...
        !isfinite(inv_waist_x) || !isfinite(inv_waist_y) || !isfinite(phase_scale) ||
    ...
    for (size_t row = 0U; row < height; ++row) {
        ...
        sim_accel_scale_inplace_real(state->vdsp_u, width, inv_waist_x);
        sim_accel_scale_inplace_real(state->vdsp_v, width, inv_waist_y);
        ...
        sim_accel_copy_scale_real(state->vdsp_value, state->vdsp_work, width, -0.5, false);
        vvexp(state->vdsp_u, state->vdsp_work, &vforce_len);

        optical_vortex_rho_factor(
            state->vdsp_value, state->vdsp_work, state->vdsp_v, width, abs_charge);
        vDSP_vmulD(state->vdsp_u, 1, state->vdsp_work, 1, state->vdsp_u, 1, len);
...
static void optical_vortex_destroy(void* state_ptr) {
    SimStimulusOpticalVortexState* state = (SimStimulusOpticalVortexState*) state_ptr;
#if defined(SIM_HAVE_VDSP)
    if (state != NULL) {
...
static SimResult optical_vortex_step(void*               state_ptr,
...
#if defined(SIM_HAVE_VDSP)
    if (optical_vortex_try_vdsp_linear_rows(state,
                                            field,
                                            is_complex,
    ...
    if (!is_complex) {
        ...
        for (size_t i = 0U; i < count; ++i) {
            ...
            optical_vortex_map_coord(&state->config, x, y, t, &u, &v);
            optical_vortex_eval_uv(&state->config, u, v, t, &re, &im);
            (void) im;
...
SimResult sim_add_stimulus_optical_vortex_operator(struct SimContext*                    context,
    ...
    SimStimulusOpticalVortexConfig local = { 0 };
    local.charge                         = 1;
    if (config != NULL) {
        local = *config;
    }

    optical_vortex_normalize(&local);
    local.scale_by_dt =
        sim_operator_resolve_scale_by_dt(context,
                                         "stimulus_optical_vortex",
                                         (config != NULL),
                                         (config != NULL) ? config->scale_by_dt : true);

    SimStimulusOpticalVortexState* state =
        (SimStimulusOpticalVortexState*) calloc(1U, sizeof(*state));
    if (state == NULL) {
        return SIM_RESULT_OUT_OF_MEMORY;
    }

    state->config = local;
    optical_vortex_refresh_symbolic(state);

    char name[SIM_OPERATOR_NAME_MAX + 1U];
    sim_operator_make_unique_name(name, sizeof(name), "stimulus_optical_vortex");
    ...
    info.abstract_id       = "stimulus_optical_vortex";
    sim_operator_info_set_schema_identity(&info, "stimulus_optical_vortex");
    ...
    SimSplitDescriptor desc = { .name          = name,
                                ...
                                .substep_count = 1U,
                                .state         = state,
                                .symbolic      = optical_vortex_symbolic,
                                .destroy       = optical_vortex_destroy,
                                .info          = info,
                                .config        = op_config,
                                .scratch       = { 0U, 0U } };
...
SimResult sim_stimulus_optical_vortex_config(struct SimContext*              context,
    ...
    SimStimulusOpticalVortexState* state = (SimStimulusOpticalVortexState*) sim_split_state(op);
    if (state == NULL) {
...
SimResult sim_stimulus_optical_vortex_update(struct SimContext*                    context,
    ...
    SimStimulusOpticalVortexConfig local = state->config;
    if (config != NULL) {
        local = *config;
    }
