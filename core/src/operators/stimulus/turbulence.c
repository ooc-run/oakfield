#include "oakfield/operators/stimulus/turbulence.h"
...
#define STIM_TURBULENCE_MAX_OCTAVES 16U

typedef struct SimStimulusTurbulenceState {
    SimStimulusTurbulenceConfig       config;
    double*                           phases;
    unsigned int                      allocated_octaves;
    SimStimulusStaticCache            cache;
    SimStimulusOctaveNoiseVdspBuffers vdsp;
    char                              symbolic[160];
} SimStimulusTurbulenceState;

static void turbulence_normalize(SimStimulusTurbulenceConfig* config) {
    if (config == NULL) {
        return;
    }
    ...
    if (config->octaves > STIM_TURBULENCE_MAX_OCTAVES) {
        config->octaves = STIM_TURBULENCE_MAX_OCTAVES;
    }
    if (config->seed == 0ULL) {
        config->seed = 1ULL;
    }
...
static void turbulence_refresh_symbolic(SimStimulusTurbulenceState* state) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    if (state == NULL) {
        return;
    }
    const SimStimulusTurbulenceConfig* cfg = &state->config;
    (void) snprintf(state->symbolic,
                    sizeof(state->symbolic),
                    "turbulence A=%.3g H=%.3g O=%u",
                    cfg->amplitude,
                    cfg->hurst,
                    cfg->octaves);
#else
...
static const char* turbulence_symbolic(const void* state_ptr) {
#if OAKFIELD_ENABLE_SYMBOLIC_KERNELS
    const SimStimulusTurbulenceState* state = (const SimStimulusTurbulenceState*) state_ptr;
...
static void turbulence_eval_base(const SimStimulusTurbulenceState* state,
                                 double                            u,
                                 double                            hurst,
                                 double                            lacunarity,
                                 ...
                                 double*                           out_re,
                                 double*                           out_im) {
    sim_stimulus_octave_noise_eval_base(SIM_STIMULUS_OCTAVE_NOISE_TURBULENCE,
...
static void turbulence_destroy(void* state_ptr) {
    SimStimulusTurbulenceState* state = (SimStimulusTurbulenceState*) state_ptr;
    if (state == NULL) {
        return;
    }
...
static SimResult turbulence_ensure_octaves(SimStimulusTurbulenceState* state) {
    if (state == NULL) {
...
static bool turbulence_can_use_static_cache(const SimStimulusTurbulenceState* state) {
    return state != NULL && sim_stimulus_coord_is_time_invariant(&state->config.coord);
}

static SimResult turbulence_fill_static_cache(void*                               userdata,
                                              const SimStimulusStaticCacheLayout* layout,
                                              bool                                need_imag,
                                              double*                             out_real,
                                              double*                             out_imag) {
    SimStimulusTurbulenceState* state = (SimStimulusTurbulenceState*) userdata;
    ...
    for (size_t idx = 0U; idx < layout->count; ++idx) {
        ...
        if (separable) {
            ...
            double fy_re = 0.0;
            double fy_im = 0.0;
            turbulence_eval_base(state, sample_x, hurst, lacunarity, octaves, &fx_re, &fx_im);
            turbulence_eval_base(state, sample_y, hurst, lacunarity, octaves, &fy_re, &fy_im);
            ...
            turbulence_eval_base(state, u, hurst, lacunarity, octaves, &base_re, &base_im);
        }
...
static SimResult turbulence_ensure_static_cache(SimStimulusTurbulenceState*         state,
                                                const SimStimulusStaticCacheLayout* layout,
                                                bool                                need_imag) {
    ...
    SimResult prep = turbulence_ensure_octaves(state);
    if (prep != SIM_RESULT_OK) {
        return prep;
    }

    return sim_stimulus_static_cache_ensure(
        &state->cache, layout, need_imag, turbulence_fill_static_cache, state);
}
...
static SimResult turbulence_kernel_value(SimStimulusTurbulenceState* state,
    ...
    SimStimulusTurbulenceConfig* cfg = &state->config;
    ...
    if (turbulence_can_use_static_cache(state)) {
        SimResult prep = turbulence_ensure_static_cache(state, &layout, use_imag);
        if (prep != SIM_RESULT_OK) {
            return prep;
        }
        double value =
...
static SimResult turbulence_ir_eval(void*           userdata,
                                    const KernelIR* kernel,
                                    size_t          element_index,
                                    size_t          component,
                                    double*         out_value) {
    ...
    SimStimulusTurbulenceState* state = (SimStimulusTurbulenceState*) userdata;
    return turbulence_kernel_value(state, kernel, element_index, false, out_value);
}

static SimResult turbulence_ir_eval_imag(void*           userdata,
                                         const KernelIR* kernel,
                                         size_t          element_index,
                                         size_t          component,
                                         double*         out_value) {
    ...
    SimStimulusTurbulenceState* state = (SimStimulusTurbulenceState*) userdata;
    return turbulence_kernel_value(state, kernel, element_index, true, out_value);
}

static SimResult turbulence_step(void*               state_ptr,
    ...
    if (turbulence_can_use_static_cache(state)) {
        SimResult prep = turbulence_ensure_static_cache(state, &layout, is_complex);
        if (prep != SIM_RESULT_OK) {
            return prep;
        }
    ...
    if (sim_stimulus_octave_noise_try_vdsp_rows(SIM_STIMULUS_OCTAVE_NOISE_TURBULENCE,
                                                &state->vdsp,
                                                &cfg->coord,
                                                state->phases,
                                                cfg->amplitude,
...
SimResult sim_add_stimulus_turbulence_operator(struct SimContext*                 context,
    ...
    SimStimulusTurbulenceConfig local = { 0 };
    if (config != NULL) {
        local = *config;
    }
    ...
    if (local.seed == 0ULL) {
        local.seed = sim_seed_derive(sim_context_seed(context),
                                     sim_seed_tag("stimulus_turbulence"),
                                     sim_context_operator_count(context));
    }

    turbulence_normalize(&local);
    local.scale_by_dt =
    ...
    SimStimulusTurbulenceState* state =
        (SimStimulusTurbulenceState*) calloc(1U, sizeof(SimStimulusTurbulenceState));
    if (state == NULL) {
    ...
    turbulence_refresh_symbolic(state);

    char name[SIM_OPERATOR_NAME_MAX + 1U];
    sim_operator_make_unique_name(name, sizeof(name), "stimulus_turbulence");
    ...
    info.abstract_id       = "stimulus_turbulence";
    sim_operator_info_set_schema_identity(&info, "stimulus_turbulence");
    ...
    if (sim_operator_should_register_kernel_for_schema(
            context, &op_config, 0ULL, SIM_DET_NONE, "stimulus_turbulence")) {
        ...
        if (field != NULL) {
            ...
            if (builder != NULL) {
                ...
                SimIRNodeId sample_node = sim_ir_builder_stateful(
                    builder, turbulence_ir_eval, state, "stimulus_turbulence");
                if (is_complex && sample_node != SIM_IR_INVALID_NODE) {
                    SimIRNodeId sample_im = sim_ir_builder_stateful(
                        builder, turbulence_ir_eval_imag, state, "stimulus_turbulence_im");
                ...
                if (field_node != SIM_IR_INVALID_NODE && sample_node != SIM_IR_INVALID_NODE &&
                    ...
                    kdesc.name                  = name;
                    kdesc.evaluate              = NULL;
                    kdesc.destroy               = turbulence_destroy;
    ...
    SimSplitDescriptor desc = { .name          = name,
                                ...
                                .substep_count = 1U,
                                .state         = state,
                                .symbolic      = turbulence_symbolic,
                                .destroy       = turbulence_destroy,
                                .info          = info,
                                .config        = op_config,
                                .scratch       = { 0U, 0U } };
...
SimResult sim_stimulus_turbulence_config(struct SimContext*           context,
    ...
    SimStimulusTurbulenceState* state = NULL;
    if (op->kernel != NULL) {
        state = (SimStimulusTurbulenceState*) sim_operator_payload(op);
    } else {
        state = (SimStimulusTurbulenceState*) sim_split_state(op);
    }
    if (state == NULL) {
...
SimResult sim_stimulus_turbulence_update(struct SimContext*                 context,
    ...
    SimStimulusTurbulenceConfig local = state->config;
    if (config != NULL) {
        local = *config;
    }
    ...
    if (local.seed == 0ULL) {
        local.seed = sim_seed_derive(
            sim_context_seed(context), sim_seed_tag("stimulus_turbulence"), operator_index);
    }

    turbulence_normalize(&local);
    local.scale_by_dt = sim_operator_resolve_scale_by_dt(
        context, sim_operator_schema_key_or(op, "stimulus_turbulence"), true, local.scale_by_dt);
    state->config            = local;
    state->allocated_octaves = 0U;
