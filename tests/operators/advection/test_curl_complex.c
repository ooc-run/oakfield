/*
 * Migrated advection operator coverage for complex curl contracts.
 */
...
static SimComplexDouble derivative_periodic(const SimComplexDouble *data, size_t height,
                                            ...
                                            SimCurlStencil stencil, double spacing) {
...
int main(void) {
    ...
    for (size_t y = 0U; y < height; ++y) {
        for (size_t x = 0U; x < width; ++x) {
            ...
            vx[index].re = sin(theta_y) + 0.2 * cos(theta_x);
            vx[index].im = 0.5 * cos(theta_y) - 0.25 * sin(theta_xy);
            vy[index].re = cos(theta_x) + 0.3 * sin(theta_xy);
            vy[index].im = sin(theta_x) - 0.4 * cos(theta_y);
        }
    }
    ...
    cfg.axis_x = 1U;
    cfg.axis_y = 0U;
    cfg.stencil = SIM_CURL_STENCIL_CENTRAL_4;
    cfg.boundary = SIM_IR_BOUNDARY_PERIODIC;
    cfg.accumulate = false;
    cfg.scale_by_dt = false;

    if (sim_add_curl_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_add_curl_operator\n");
        sim_context_destroy(&ctx);
        return 1;
    }

    SimCurlOperatorConfig fetched;
    memset(&fetched, 0, sizeof(fetched));
    if (sim_curl_config(&ctx, op_index, &fetched) != SIM_RESULT_OK ||
        fetched.stencil != SIM_CURL_STENCIL_CENTRAL_4 ||
        fetched.boundary != SIM_IR_BOUNDARY_PERIODIC) {
        fprintf(stderr, "FAIL: sim_curl_config\n");
        sim_context_destroy(&ctx);
        return 1;
    }

    if (sim_curl_update(&ctx, op_index, &fetched) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_curl_update\n");
        sim_context_destroy(&ctx);
        return 1;
    }
    ...
    for (size_t y = 0U; y < height; ++y) {
        for (size_t x = 0U; x < width; ++x) {
            size_t index = y * width + x;
            SimComplexDouble dvy_dx =
                derivative_periodic(vy, height, width, y, x, 1U, cfg.stencil, spacing_x);
            SimComplexDouble dvx_dy =
                derivative_periodic(vx, height, width, y, x, 0U, cfg.stencil, spacing_y);
            double expected_re = dvy_dx.re - dvx_dy.re;
            double expected_im = dvy_dx.im - dvx_dy.im;
