...
static void eval_base(const double *phases, double u, double hurst, double lacunarity,
    ...
    double amp = 1.0;
    double freq = 1.0;
    double weight_re = 1.0;
    double weight_im = 1.0;
    double sum_re = 0.0;
    double sum_im = 0.0;
    ...
    for (unsigned int o = 0U; o < octaves; ++o) {
        ...
        double basis_im = sin(theta);

        sum_re += amp * weight_re * basis_re;
        sum_im += amp * weight_im * basis_im;

        weight_re *= 0.5 * (1.0 + fabs(basis_re));
        weight_im *= 0.5 * (1.0 + fabs(basis_im));
        freq *= lacunarity;
        amp *= amp_decay;
    }
...
static int run_hybrid_fbm_case(const SimStimulusHybridFbmConfig *cfg_input, double dt, int steps,
                               double tol) {
    SimContext ctx;
    ...
    SimStimulusHybridFbmConfig cfg;
    ...
    size_t op_index = 0U;
    if (sim_add_stimulus_hybrid_fbm_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_add_stimulus_hybrid_fbm_operator\n");
        sim_context_destroy(&ctx);
        return 0;
    }

    SimStimulusHybridFbmConfig normalized;
    memset(&normalized, 0, sizeof(normalized));
    if (sim_stimulus_hybrid_fbm_config(&ctx, op_index, &normalized) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_stimulus_hybrid_fbm_config\n");
        sim_context_destroy(&ctx);
        return 0;
    }
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        for (size_t i = 0U; i < count; ++i) {
            ...
            eval_base(phases, x, normalized.hurst, normalized.lacunarity, normalized.octaves,
                      &fx_re, &fx_im);
            eval_base(phases, y, normalized.hurst, normalized.lacunarity, normalized.octaves,
                      &fy_re, &fy_im);
...
int main(void) {
    ...
    cfg.coord.origin_x = -0.85;
    cfg.coord.origin_y = 0.12;
    cfg.coord.spacing_x = 0.27;
    cfg.coord.spacing_y = 0.21;

    return run_hybrid_fbm_case(&cfg, 0.125, 4, kTol) ? 0 : 1;
}
