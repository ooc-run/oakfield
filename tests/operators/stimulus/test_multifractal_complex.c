...
static void eval_base(const double *phases, double u, double hurst, double lacunarity,
    ...
    for (unsigned int o = 0U; o < octaves; ++o) {
        ...
        product_re *= 1.0 + 0.5 * amp * cos(theta);
        product_im *= 1.0 + 0.5 * amp * sin(theta);
        freq *= lacunarity;
        amp *= amp_decay;
    }

    if (out_re != NULL) {
        *out_re = product_re - 1.0;
    }
    if (out_im != NULL) {
        *out_im = product_im - 1.0;
    }
}

static int run_multifractal_case(const SimStimulusMultifractalConfig *cfg_input, double dt,
    ...
    SimStimulusMultifractalConfig cfg;
    ...
    size_t op_index = 0U;
    if (sim_add_stimulus_multifractal_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_add_stimulus_multifractal_operator\n");
        sim_context_destroy(&ctx);
        return 0;
    }

    SimStimulusMultifractalConfig normalized;
    memset(&normalized, 0, sizeof(normalized));
    if (sim_stimulus_multifractal_config(&ctx, op_index, &normalized) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_stimulus_multifractal_config\n");
        sim_context_destroy(&ctx);
        return 0;
    }
...
int main(void) {
    ...
    cfg.coord.origin_x = -0.92;
    cfg.coord.origin_y = 0.11;
    cfg.coord.spacing_x = 0.24;
    cfg.coord.spacing_y = 0.19;

    return run_multifractal_case(&cfg, 0.125, 4, kTol) ? 0 : 1;
}
