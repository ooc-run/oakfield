...
static void optical_vortex_map_coord(const SimStimulusOpticalVortexConfig *cfg, double x, double y,
...
static void optical_vortex_eval(const SimStimulusOpticalVortexConfig *cfg, double u, double v,
...
static int run_optical_vortex_case(const char *label,
                                   const SimStimulusOpticalVortexConfig *cfg_input, size_t rank,
    ...
    SimStimulusOpticalVortexConfig cfg;
    ...
    if (sim_add_stimulus_optical_vortex_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL[%s]: sim_add_stimulus_optical_vortex_operator\n", label);
        sim_context_destroy(&ctx);
        return 0;
    }

    SimStimulusOpticalVortexConfig normalized;
    memset(&normalized, 0, sizeof(normalized));
    if (sim_stimulus_optical_vortex_config(&ctx, op_index, &normalized) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL[%s]: sim_stimulus_optical_vortex_config\n", label);
        sim_context_destroy(&ctx);
        return 0;
    }
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        for (size_t i = 0U; i < count; ++i) {
            ...
            optical_vortex_map_coord(&normalized, x, y, t, &u, &v);
            optical_vortex_eval(&normalized, u, v, t, &re, &im);
...
int main(void) {
    {
        ...
        cfg.coord.origin_x = -1.3;
        cfg.coord.origin_y = -1.1;
        cfg.coord.spacing_x = 0.46;
        cfg.coord.spacing_y = 0.39;

        if (!run_optical_vortex_case("separable_small", &cfg, 2U, kShape, 0.05, 3, kTol)) {
            return 1;
        }
    }

    {
        ...
        if (!run_optical_vortex_case("angle_wide", &cfg, 2U, kShapeWide, 0.03, 3, 1.0e-7)) {
            return 1;
        }
    }

    {
        ...
        cfg.coord.spiral_phase = 0.16;
        cfg.coord.spiral_angular_velocity = 0.11;

        if (!run_optical_vortex_case("spiral_small", &cfg, 2U, kShape, 0.03, 2, kTol)) {
            return 1;
        }
    }
