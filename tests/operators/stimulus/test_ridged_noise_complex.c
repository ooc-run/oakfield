...
static const double kRidgedProfileMean = 1.5 - 4.0 / M_PI;
...
static double ridged_profile(double base_value) {
    double ridge = 1.0 - fabs(base_value);
    return ridge * ridge - kRidgedProfileMean;
}
...
static void eval_base(const double *phases, double u, double hurst, double lacunarity,
    ...
    for (unsigned int o = 0U; o < octaves; ++o) {
        ...
        sum_re += amp * ridged_profile(cos(theta));
        sum_im += amp * ridged_profile(sin(theta));
        freq *= lacunarity;
        amp *= amp_decay;
    }
...
static int run_ridged_noise_case(const SimStimulusRidgedNoiseConfig *cfg_input, double dt,
    ...
    SimStimulusRidgedNoiseConfig cfg;
    ...
    size_t op_index = 0U;
    if (sim_add_stimulus_ridged_noise_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_add_stimulus_ridged_noise_operator\n");
        sim_context_destroy(&ctx);
        return 0;
    }

    SimStimulusRidgedNoiseConfig normalized;
    memset(&normalized, 0, sizeof(normalized));
    if (sim_stimulus_ridged_noise_config(&ctx, op_index, &normalized) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_stimulus_ridged_noise_config\n");
        sim_context_destroy(&ctx);
        return 0;
    }
...
int main(void) {
    ...
    return run_ridged_noise_case(&cfg, 0.125, 4, kTol) ? 0 : 1;
}
