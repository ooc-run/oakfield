...
static const size_t kShape2DSmall[] = {5U, 6U};
static const size_t kShape2DWide[] = {4U, 96U};
static const size_t kShape1D[] = {8U};
...
static void traveling_wave_packet_eval(const SimStimulusTravelingWavePacketConfig *cfg, double u,
...
static int run_packet_case(const char *label, const SimStimulusTravelingWavePacketConfig *cfg_input,
    ...
    SimStimulusTravelingWavePacketConfig cfg;
    ...
    if (sim_add_stimulus_traveling_wave_packet_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL[%s]: sim_add_stimulus_traveling_wave_packet_operator\n", label);
        sim_context_destroy(&ctx);
        return 0;
    }

    SimStimulusTravelingWavePacketConfig normalized;
    memset(&normalized, 0, sizeof(normalized));
    if (sim_stimulus_traveling_wave_packet_config(&ctx, op_index, &normalized) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL[%s]: sim_stimulus_traveling_wave_packet_config\n", label);
        sim_context_destroy(&ctx);
        return 0;
    }
    ...
    for (int step = 0; step < steps; ++step) {
        ...
        for (size_t i = 0U; i < count; ++i) {
            ...
            traveling_wave_packet_map_coord(&normalized, x, y, t, &u, &v);
            traveling_wave_packet_eval(&normalized, u, v, t, &re, &im);
...
int main(void) {
    {
        ...
        cfg.coord.spacing_x = 0.24;
        cfg.coord.spacing_y = 0.21;
        if (!run_packet_case("axis_small", &cfg, 2U, kShape2DSmall, 0.07, 3, kTol2D)) {
            return 1;
        }
    }

    {
        ...
        cfg.coord.spacing_y = 0.09;
        cfg.coord.velocity_x = 0.03;
        cfg.coord.velocity_y = -0.02;
        if (!run_packet_case("angle_wide", &cfg, 2U, kShape2DWide, 0.03, 4, 1.0e-7)) {
            return 1;
        }
    }

    {
        ...
        if (!run_packet_case("axis_1d", &cfg, 1U, kShape1D, 0.05, 4, kTol1D)) {
            return 1;
        }
    }
