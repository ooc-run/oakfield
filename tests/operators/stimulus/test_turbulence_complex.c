...
static const double kAbsMean = 2.0 / M_PI;
...
static void eval_base(const double *phases, double u, double hurst, double lacunarity,
    ...
    for (unsigned int o = 0U; o < octaves; ++o) {
        ...
        sum_re += amp * (fabs(cos(theta)) - kAbsMean);
        sum_im += amp * (fabs(sin(theta)) - kAbsMean);
        freq *= lacunarity;
        amp *= amp_decay;
    }
...
static int run_turbulence_case(const SimStimulusTurbulenceConfig *cfg_input, double dt, int steps,
                               double tol) {
    SimContext ctx;
    ...
    SimStimulusTurbulenceConfig cfg;
    ...
    size_t op_index = 0U;
    if (sim_add_stimulus_turbulence_operator(&ctx, &cfg, &op_index) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_add_stimulus_turbulence_operator\n");
        sim_context_destroy(&ctx);
        return 0;
    }

    SimStimulusTurbulenceConfig normalized;
    memset(&normalized, 0, sizeof(normalized));
    if (sim_stimulus_turbulence_config(&ctx, op_index, &normalized) != SIM_RESULT_OK) {
        fprintf(stderr, "FAIL: sim_stimulus_turbulence_config\n");
        sim_context_destroy(&ctx);
        return 0;
    }
...
int main(void) {
    ...
    cfg.coord.velocity_x = 0.17;
    cfg.coord.velocity_y = -0.09;
    cfg.coord.spacing_x = 0.3;
    cfg.coord.spacing_y = 0.22;

    return run_turbulence_case(&cfg, 0.125, 4, kTol) ? 0 : 1;
}
